...
- DB 입력은 `NamedParameterJdbcTemplate.batchUpdate()`를 활용한다.
...
- UPDATE와 DELETE 구문의 WHERE 절이 DB 인덱스를 잘 타는지 확인한다.
...
- 애너테이션으로 콜백을 표시한 리스너는 `chunk()` 뒤에서 `listener()`로 등록한다. `chunk()` 앞에 등록하면 `@BeforeStep`, `@AfterStep`만 연결되고 청크·아이템 수준 애너테이션은 에러 없이 무시된다.
...
- `@Bean` 없이 스프링 배치 구성 요소를 생성하는 메서드는 이름을 `build`로 시작하고, 분기 구성에 쓸 때는 지역 변수에 담은 인스턴스를 재사용한다.
