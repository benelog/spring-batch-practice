...
- 목(mock) 라이브러리 대신 인터페이스를 구현한 가짜 객체를 만들어 검증한다.
...
- SQL 구문은 `AccessLogSql`처럼 상수를 모은 클래스에 선언한다. 조건 값은 파라미터 바인딩으로 전달하고, 이름 붙은 파라미터(`:username`)를 지원하는 클래스에서는 '?' 대신 이름 붙은 파라미터를 쓴다.
...
- `skip()` 대상은 `Exception` 같은 넓은 범위가 아니라 발생을 확인한 구체적인 예외 클래스만 지정한다.
- 스킵할 예외와 `skipLimit` 값이 지정되지 않았으면 추측으로 채우지 말고 사람에게 먼저 묻는다.
- 스킵을 허용한 스텝은 건너뛴 아이템을 확인할 수 있게 기록을 남긴다.
- 재시도는 일시적이라고 확인된 실패에만 걸고, 멱등하지 않은 API를 호출하는 구간에는 걸지 않는다. HTTP GET 요청은 멱등하다고 가정하고 그 외는 사람에게 확인해라.
...
- `SkipListener`와 `RetryListener`는 `faultTolerant()` 호출 뒤에 등록한다.
...
- 스프링 부트 모듈은 'admin-cli'와 'admin-web'에서만 의존한다.
- 라이브러리 버전을 개별 모듈의 build.gradle에 직접 선언하지 않는다. 스프링 부트 버전은 최상위 gradle.properties에서, 공통 의존성은 buildSrc의 공통 플러그인에서 관리한다.
- 다른 모듈에 노출할 의존성만 `api`로, 그 외에는 `implementation`으로 선언한다.
- 새 잡의 설정 클래스는 잡의 성격에 맞는 잡 그룹 모듈에 두고, 그 모듈의 JobGroupContexts 클래스의 `@Import` 목록에 추가한다. 컴포넌트 스캔으로 등록하지 않는다.

## 배치 모니터링

- 커스텀 메트릭 이름은 'batch.step.written'처럼 점(.)으로 구분해서 짓고, 'job'과 'step' 태그를 붙인다.
